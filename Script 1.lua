...
local function vapeGithubRequest(scripturl, isImportant)
    ...
    local shouldLoadFromCustom = false
    for _, customFile in pairs(customFiles) do
        if scripturl == customFile then
            shouldLoadFromCustom = true
            break
        end
    end
    
    -- Wenn diese Datei von deinem Repository geladen werden soll
    if shouldLoadFromCustom then
        local customRepo = shared.MyGitHubRepo or MY_GITHUB_REPO
        local customBranch = shared.MyGitHubBranch or MY_GITHUB_BRANCH
    ...
    -- Alle anderen Dateien werden vom Original-Repository geladen
